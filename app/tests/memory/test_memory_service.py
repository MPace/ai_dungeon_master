...
class TestMemoryService:
    ...
    def test_find_similar_memories_with_vector_search(self, mock_db):
        ...
        # Verify aggregate was called with the right pipeline
        pipeline_arg = mock_db.memory_vectors.aggregate.call_args[0][0]
        assert len(pipeline_arg) > 0
        assert any('$search' in stage for stage in pipeline_arg)
        assert any('session_id' in stage.get('$match', {}) for stage in pipeline_arg)
    ...
    def test_find_similar_memories_fallback(self, mock_db):
        ...
        with patch.object(MemoryService, '_cosine_similarity') as mock_cosine:
            mock_cosine.side_effect = [0.95, 0.85]  # Values for our two mock memories
