...
class TestSummarizationService:
    ...
    def test_summarize_memories(self, summarization_service, mock_db, mock_embedding_service):
        ...
        with patch('app.services.memory_service.MemoryService.create_memory_summary') as mock_create_summary:
            ...
            assert result['summary'] == mock_memory
            ...
            assert "Memory 1: First memory" in summarizer_call_arg
            assert "Memory 2: Second memory" in summarizer_call_arg
            
            # Verify embedding service was called
            assert mock_embedding_service.generate_embedding.called
            
            # Verify create_memory_summary was called with correct args
            mock_create_summary.assert_called_with(
                memory_ids=['memory1', 'memory2'],
                summary_content="This is a summarized version of the text.",
                summary_embedding=mock_embedding_service.generate_embedding.return_value,
                session_id=session_id,
                character_id='char1',
                user_id='user1'
            )
            ...
            assert mock_db.memory_vectors.update_one.call_count == 2
            for memory_id in ['memory1', 'memory2']:
                mock_db.memory_vectors.update_one.assert_any_call(
                    {'memory_id': memory_id},
                    {'$set': {'is_summarized': True, 'summary_id': 'summary1'}}
                )
    ...
    def test_trigger_summarization_if_needed(self, summarization_service):
        ...
        with patch('app.services.memory_service.MemoryService.check_summarization_triggers') as mock_check:
            ...
            with patch.object(summarization_service, 'summarize_memories') as mock_summarize:
                ...
                mock_summarize.assert_called_with(session_id, time_window=timedelta(minutes=60))
